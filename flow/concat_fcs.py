...
# get flow file paths
flow_path = os.path.abspath(r"./")
concat_head, concat_tail = os.path.split(flow_path)
concat_path = os.path.join(concat_head, concat_tail + "_concat.fcs")
pytest_confirm = "PYTEST_CURRENT_TEST" in os.environ
...
consens_chans = {}
flow_paths = sorted(get_files(path=flow_path, pat="*.fcs", anti=""))
...
concat_data = fio.FlowData(concat_path, only_text=True)
print(
    f'"{concat_data.name}"\n'
